...
public class ArrayExample {
	 ...
	 *   각 변수들은 index를 이용하여 구분함.   ex) 축구팀 명 - 선수번호
	 */
	...
	public void ex2() {
		...
		// stack 영역은 컴파일러가 알아서 초기화 X(선언하고 비어있는 것만 가능)
		// heap  영역은 컴파일러가 알아서 초기화 O
		int a;
		// System.out.println(a);
		// The local variable a may not have been initialized 초기화되지 않았다
	...
	public void ex3() {
		...
		// 배역의 주소가 출력됨 [D@6442b0a6 (해쉬코드)
		// 왜? heightArr 참조 변수에는 배열의 시작 주소가 저장되어 있기 때문
		
		// Array.toString(배열명) : 배열에 저장된 모든 값을 한 줄로 간단히 출력
	...
	public void ex4() {
		// ArrayIndexOutOfBoundsException : 배열 인덱스 범위 초과 (i<=arr 작성시)
		int[] arr = new int[3];
		...
		// Array.toString(arr) : 배열에 저장된 모든 값을 한 줄로 간단히 출력
		// 확인만 하는 용도, 코드에 활용 X
		...
		for(int i=0; i < arr.length; i++) {
			...
			if(arr[i] == search) { // arr[i] 값이 입력 받은 값과 같다면
				...
				break; // 중복 데이터가 없으면 추가 검색을 하지 않아도 됨
					  // 예) 500을 앞에서 발견하면 뒤는 검사하지 않도록
			}
		}
		
		if(flag) {  // false면 수행하지 않기 때문에 true일 때만 존재하지 않는다고 출력
		...
		for(int i=0; i < lotto.length; i++) {
			...
			for(int x=0; x < i; x++) {
				if(lotto[x] == ran) {
					...
					break; // 추가 검사가 불필요하기 때문에 멈춤(뒤에 출력되었으면 이미 검사가 완료되었을 것)
				}
			}
		}
...
//		System.arraycopy(arr, 0, copyArr, 0, arr.length);
	...
	public void ex11() {
		...
		if(arr1 != null) { // arr1이 참조하는 배열이 있을 떄에만 수행
