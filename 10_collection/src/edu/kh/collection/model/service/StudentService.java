...
public class StudentService {
	...
	public StudentService() {
		...
		stdList.add(new Student("김길자", 17, "서울시 중구", 'F', 90));
		stdList.add(new Student("이순자", 19, "경기도 부천시", 'F', 100));
	}
	...
	public boolean addStudent(String name, int age, String address, char gender, int score) {
		...
		return stdList.add(std);  // 호출한 곳(view의 if문)으로 반환한다
	}
		// 제네릭 확인 테스트
...
//		List<Integer> test3 = new ArrayList<Integer>();
	...
	public boolean updateStudent(int idx, String address, int score) {
		...
		// 입력 받은 idx가 stdList에 실재 존재하는 요소의 인덱스 범위를 넘어선 경우
	...
	public List<Student> selectStudent1(String name) {
		...
		for( Student s : stdList ) {
			...
			// 검색한 이름과 학생 이름이 같은 경우(매개변수로 전달받은 name)
			if(s.getName().equals(name)) {
				resultList.add(s); // 결과 추가
			}
		}
		return resultList;
	}
