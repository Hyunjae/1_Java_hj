...
import edu.kh.poly.ex2.model.vo.JHJCalculator;
...
//		f1.breath(); // 동적바인딩 된 부분 X, 그냥 Fish 내부에 존재
		...
		// 추상 매서드였던 breath() 호출 가능 여부
		a1.breath();
		a2.breath();
		// 실행 전에는 Animal(부모)의 breath()와 연결 된 것으로 보임 (정적 바인딩)
		...
		// 모든 자식이 강제 오버라이딩한 breath()라는 공통적인 메서드를 
...
//		KH k1 = new KH();  //Cannot instantiate the type KH
		...
		arr[1] = new HGD(); // 업캐스팅
		// 부모타입 참조 = 자식객체;
		// 부모 : KH 인터페이스 == 인터페이스도 부모 참조변수로 사용 가능
	...
	public void ex3() {
		...
		Calculator cal = new JHJCalculator();
		// JHJCalculator 부분만 바꿔도 다른 코드 실행 가능
