...
public class Example {
	...
	public void ex2() {
		...
		try {
			...
			// catch가 실행 안되면 정상수행, input이 출력됨
		
			// 예외 강제 발생
			// == 예외 객체를 생성해서 던짐
			throw new IOException();  // 예외객체 생성 + 던진다
				
		} catch(IOException e) {
			// catch : try 구문 내에서 예외 객체가 던져진 경우
			// 		   매개변수(참조형)를 이용해서 해당 객체를 잡는다.
			//         (잡는다 = 매개변수에 예외객체의 주소를 저장해서 참조)
	...
	public void ex3() {
		...
		// Resource leak: 'sc' is never closed : 닫으라는 경고
		...
		try {
			...
			// Exception catch 구문을 앞에 추가했을 경우
			// Unreachable catch block for ArithmeticException. 
			// It is already handled by the catch block for Exception
			
		} catch(InputMismatchException e) { // catch문 다중작성 가능
				// catch문이 여러개면 위에서부터 순서대로 발생한 예외를 검사하여
				// 알맞은 매개변구를 가진 catch에서 처리한다.(if~else와 비슷)
			...
			sc.close(); // 위에 쓰면 안되고 무조건 마지막에 실행
		}
	}
