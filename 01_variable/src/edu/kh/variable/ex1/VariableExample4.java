...
public class VariableExample4 {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 형변환은 왜 필요한가?
		 * -> 컴퓨터의 값 처리 원칙을 위배하는 경우, 이를 해결하기 위해서
		 * 
		 * 
		 * 자동 형변환
		...
		System.out.println("num1 + num2 = " + (num1 + num2) );  //long형
		...
		System.out.println(result);  //double형
		...
		double temp = 3.14;
		int result3 = (int)temp; // 값의 범위가 큰 double -> int로 강제 형변환
		...
		System.out.println( iNum1 / iNum2 );
						//    1   /   2   =  0     결과가 0.5지만 double형이 아닌 int형이기 때문에 소수점 버림
						//   int  /  int  = int
		
		// 강제 형변환 / 자동 형변환을 동시에 이용
		System.out.println( (double)iNum1 / iNum2 );
						//      1.0    /   2.0   =  0.5      double로 강제형변환 해주면
						//    double   /   int   =  double   int가 double로 자동형변환 된다
					    //              -> double
